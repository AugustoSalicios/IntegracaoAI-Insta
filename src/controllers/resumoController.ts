...
import { salvarNoBanco } from "../services/dbService.js";
...
export const criarResumoAi = async(req: Request, res: Response) => {
    try{
        ...
        if(!textoOriginal){
            return res.status(400).json({erro: 'Por favor envie campo textoOriginal'});
        }

        const resumoGerado = await gerarResumoAI(textoOriginal);
        const registroSalvo = await salvarNoBanco(textoOriginal, resumoGerado);
